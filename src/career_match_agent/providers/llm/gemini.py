...
class GeminiStructuredLLMProvider:
    ...
    async def generate_structured(self, *, system_prompt: str, user_prompt: str, response_model: type[StructuredOutputT]) -> StructuredOutputT:
        try:
            response = (await self._client.aio.models.generate_content(model=self.model_name, contents=user_prompt, config=types.GenerateContentConfig(system_instruction=system_prompt,
                                                                       temperature=0, response_mime_type=("application/json"), response_schema=response_model)))
